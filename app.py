...
feature_names=df.drop(columns=['cyberbullying_type']).columns.tolist()
...
def predict():
    try:
        ...
        text_input = request.form["tweet_text"]
        # features = [float(x) for x in request.form.values()]
        # final_features = np.array([features])   # shape (1, n_features)

        # Predict
        prediction = model.predict([text_input])       # numeric
        decoded_pred = le.inverse_transform(prediction)  # decoded label

        return render_template(
            "index.html",
            feature_names=feature_names,
            prediction_text=f"Cyber Bullying Type is: {decoded_pred[0]}"
        )
